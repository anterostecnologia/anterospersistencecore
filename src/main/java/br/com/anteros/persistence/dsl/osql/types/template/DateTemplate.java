...
public class DateTemplate<T extends Comparable<?>> extends DateExpression<T> implements TemplateExpression<T> {
    ...
    public static <T extends Comparable<?>> DateExpression<T> create(Class<T> type, String template) {
        return new DateTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.of());
    }
    
    public static <T extends Comparable<?>> DateExpression<T> create(Class<T> type, String template, Object one) {
        return new DateTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.of(one));
    }
    
    public static <T extends Comparable<?>> DateExpression<T> create(Class<T> type, String template, Object one, Object two) {
        return new DateTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.of(one, two));
    }
    
    public static <T extends Comparable<?>> DateExpression<T> create(Class<T> type, String template, Object... args) {
        return new DateTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.copyOf(args));
    }

    public static <T extends Comparable<?>> DateExpression<T> create(Class<T> type, Template template, Object... args) {
        return new DateTemplate<T>(type, template, ListUtils.copyOf(args));
    }
