...
public class EnumTemplate<T extends Enum<T>> extends EnumExpression<T> implements TemplateExpression<T> {
    ...
    public static <T extends Enum<T>> EnumExpression<T> create(Class<T> type, String template) {
        return new EnumTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.of());
    }
    
    public static <T extends Enum<T>> EnumExpression<T> create(Class<T> type, String template, Object one) {
        return new EnumTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.of(one));
    }
    
    public static <T extends Enum<T>> EnumExpression<T> create(Class<T> type, String template, Object one, Object two) {
        return new EnumTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.of(one, two));
    }
    
    public static <T extends Enum<T>> EnumExpression<T> create(Class<T> type, String template, Object... args) {
        return new EnumTemplate<T>(type, TemplateFactory.DEFAULT.create(template), ListUtils.copyOf(args));
    }

    public static <T extends Enum<T>> EnumExpression<T> create(Class<T> type, Template template, Object... args) {
        return new EnumTemplate<T>(type, template, ListUtils.copyOf(args));
    }
